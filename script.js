...
async function checkApiConnectivity() {
  ...
  if (!base) {
    console.warn('[API CHECK] API_BASE пуст. Задай window.API_BASE.');
    return { ok: false, reason: 'EMPTY_BASE' };
  }
  ...
  // 1) быстрый ping через OPTIONS (не меняет БД)
  try {
    const r1 = await fetchTO(url, { method: 'OPTIONS' }, 4000);
    if (!r1.ok) {
      console.error(`[API CHECK] OPTIONS ${url} → HTTP ${r1.status}`);
      return { ok: false, step: 'OPTIONS', status: r1.status };
    }
  } catch (e) {
    console.error(`[API CHECK] OPTIONS ${url} → ${e}`);
    return { ok: false, step: 'OPTIONS', err: String(e) };
  }

  // 2) реальный POST (проверка роутера и CORS)
  try {
    const body = initData
      ? { initData }
      : { user_id: USER_ID || 1, username: USERNAME || 'user' };

    const r2 = await fetchTO(url, {
      method: 'POST',
    ...
    let json = null;
    try { json = await r2.clone().json(); } catch {}

    if (r2.ok && json && json.ok) {
      console.info(`[API OK] ${base} • ${dt}ms`);
      return { ok: true, ms: dt, base, status: r2.status };
    }
    console.warn(`[API FAIL] ${base} • HTTP ${r2.status} • body=${JSON.stringify(json)}`);
    return { ok: false, step: 'POST', status: r2.status, body: json };
  } catch (e) {
    console.error(`[API DOWN] ${base} • ${e}`);
    return { ok: false, step: 'POST', err: String(e) };
  }
}

// вызов на старте и экспорт в консоль
