...
window.API_BASE="https://wiley-departure-rogers-victorian.trycloudflare.com"
...
;(function(){
  ...
  async function loadLogs() {
    ...
    } catch (err) { Notify.error('Не удалось загрузить лог'); }
  }
  ...
  byId('sendPhone')?.addEventListener('click', async () => {
    ...
    try {
      ...
      loadLogs();
    } catch (err) { Notify.error('Ошибка отправки номера'); }
  });
  ...
  byId('sendCode')?.addEventListener('click', async () => {
    ...
    try {
      // достанем id последней заявки из лога
      ...
      if (!r.ok) throw new Error(r.error || 'ERR');
      Notify.success('Код отправлен');
      loadLogs();
    } catch (err) { Notify.error('Ошибка отправки кода'); }
  });
  ...
  async function loadReport() {
    ...
    } catch (err) { Notify.error('Ошибка загрузки отчёта'); }
  }
  ...
  byId('buy-premium')?.addEventListener('click', async () => {
    ...
    } catch (_) { Notify.error('Недостаточно средств/ошибка'); }
  });
  ...
  byId('std-activate')?.addEventListener('click', async () => {
    ...
    } catch (_) { Notify.error('Ошибка активации стандарта'); }
  });
  ...
  async function spin() {
    try {
      ...
      if (!r.ok) throw new Error(r.error || 'ERR');
      buildTiles();
      const tileW = 120 + 10; // min-width + gap
  ...
  byId('wdCancel')?.addEventListener('click', async () => {
    ...
    } catch (_) { Notify.error('Отменить не удалось'); }
  });
  ...
  async function loadContests() {
    ...
    } catch (_) { Notify.error('Ошибка загрузки конкурсов'); }
  }
